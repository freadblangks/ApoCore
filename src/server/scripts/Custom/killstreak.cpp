...
class System_OnKill : public PlayerScript
{
	public:
		...
		void OnPVPKill(Player * Killer, Player * Victim)
		{
			...
			{
				//Server Broadcast
				ss << "|cFF00FF00[CCR-PvP]|cFF81CF42[" << Killer->GetName() << "]|r has ended the streak of |cFF81CFCF[" << Victim->GetName() << "]|r and has earned 1 bronze bounty!";
				sWorld->SendServerMessage(SERVER_MSG_STRING, ss.str().c_str());
				//Give Player Gold Reward (Bounty)
				Killer->SetMoney(Killer->GetMoney() + 1);
				//Negative Check && 5 Gold Death Cost for Victim
				if ((Victim->GetMoney()) <1){ Victim->SetMoney(0); }else{ Victim->SetMoney(Victim->GetMoney() - 1); }
				//Send Notification
				Victim->GetSession()->SendAreaTriggerMessage("[CCR-PvP] Your killstreak has ended! (Cost 1B)");
				//Send Message (Killing Streak Ended)
				ChatHandler(Victim->GetSession()).PSendSysMessage("|cFFFF0000[CCR-PvP] Your killstreak has ended! (Cost 1B)");
			...
			switch (KillStreakData[KillerGUID].killstreak)
			{
				case 5: case 10: case 20: case 30: case 40: case 50:
						...
						if ((Victim->GetMoney()) <1){ Victim->SetMoney(0); }else{ Victim->SetMoney(Victim->GetMoney() - 1); }
						//Send Notification
						Victim->GetSession()->SendAreaTriggerMessage("|cFFFF0000[CCR-PvP] You died, 1 bronze taken for medical costs.");
						//Send Message
						ChatHandler(Victim->GetSession()).PSendSysMessage("|cFFFF0000[CCR-PvP] You died, 1 bronze taken for medical costs.");
				break;
				...
				default :
						...
						Killer->SetMoney(Killer->GetMoney() + 1);
						Killer->GetSession()->SendAreaTriggerMessage("|cFF00FF00[CCR-PvP] You have killed enemy player! You are awarded 1 bronze for your services!");
						//Send Message
						ChatHandler(Killer->GetSession()).PSendSysMessage("|cFF00FF00[CCR-PvP] You have killed enemy player! You are awarded 1 bronze for your services!");
						//Negative Check && 50 Silver Death Cost for Victim
						if ((Victim->GetMoney()) <1){ Victim->SetMoney(0); }else{ Victim->SetMoney(Victim->GetMoney() - 1); }
						//Send Notification
						Victim->GetSession()->SendAreaTriggerMessage("|cFFFF0000[CCR-PvP] You have been killed, 1 bronze taken for medical costs.");
						//Send Message
						ChatHandler(Victim->GetSession()).PSendSysMessage("|cFFFF0000[CCR-PvP] You have been killed, 1 bronze taken for medical costs.");
				break;
			}
		}
};

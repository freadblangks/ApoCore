...
#include "Vehicle.h"
#include "../Custom/Transmogrification.h" //alfa 01.07.2014
                ...
                switch (GetSpellInfo()->Id)
                {
                    ...
                    case SPELL_COPY_WEAPON_3_AURA:
                    {
                        ...
                        if (Player* player = caster->ToPlayer())
                        {
                            ...
                            { //alfa 01.07.2014
                                if (uint32 entry = sTransmogrification->GetFakeEntry(mainItem)) //alfa 01.07.2014
                                    target->SetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID, entry); //alfa 01.07.2014
                                else //alfa 01.07.2014
                                    target->SetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID, mainItem->GetEntry()); //alfa 01.07.2014
                            } //alfa 01.07.2014
                        }
                        else
                    ...
                    case SPELL_COPY_OFFHAND_2_AURA:
                    {
                        ...
                        if (Player* player = caster->ToPlayer())
                        {
                            if (Item* offItem = player->GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_OFFHAND))
                                if (uint32 entry = sTransmogrification->GetFakeEntry(offItem)) //alfa 01.07.2014
                                    target->SetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID + 1, entry); //alfa 01.07.2014
                                else //alfa 01.07.2014
                                    target->SetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID + 1, offItem->GetEntry()); //alfa 01.07.2014
                        }
                        else
                    ...
                    case SPELL_COPY_RANGED_AURA:
                    {
                        ...
                        if (Player* player = caster->ToPlayer())
                        {
                            if (Item* rangedItem = player->GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_RANGED))
                                if (uint32 entry = sTransmogrification->GetFakeEntry(rangedItem)) //alfa 01.07.2014
                                    target->SetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID + 2, entry); //alfa 01.07.2014
                                else //alfa 01.07.2014
                                    target->SetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID + 2, rangedItem->GetEntry()); //alfa 01.07.2014
                        }
                        else

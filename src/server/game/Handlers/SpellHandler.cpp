...
void WorldSession::HandleMirrorImageDataRequest(WorldPacket& recvData)
{
    ...
    if (creator->GetTypeId() == TYPEID_PLAYER)
    {
        ...
        for (EquipmentSlots const* itr = &itemSlots[0]; *itr != EQUIPMENT_SLOT_END; ++itr)
        {
			...
			{ //alfa 01.07.2014
				if (uint32 entry = sTransmogrification->GetFakeEntry(item)) //alfa 01.07.2014
					 data << uint32(sObjectMgr->GetItemTemplate(entry)->DisplayInfoID); //alfa 01.07.2014
				else //alfa 01.07.2014
					 data << uint32(item->GetTemplate()->DisplayInfoID); //alfa 01.07.2014
			} //alfa 01.07.2014
            else
                data << uint32(0);
        }
